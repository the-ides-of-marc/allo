...
// an empty allocator.
static void test_allo_stack_alloc_unsafe_empty_allocator(void) {
  for (size_t size_i = 0; size_i < ALLO_ARR_LEN(chunk_sizes); ++size_i) {
    for (size_t align_i = 0; align_i < ALLO_ARR_LEN(aligns); ++align_i) {
      ...
      void *dest = NULL;
      status = allo_stack_alloc_unsafe(&dest, &s, chunk_sizes[size_i],
                                       aligns[align_i]);
...
static void test_allo_stack_alloc_unsafe_non_empty_allocator(void) {
...
// memory.
static void test_allo_stack_alloc_unsafe_oom(void) {
  const size_t size = 8;
  ...
  for (size_t align_i = 0; align_i < ALLO_ARR_LEN(aligns); ++align_i) {
    for (uintptr_t offset_i = 0; offset_i < ALLO_ARR_LEN(offsets); ++offset_i) {
      ...
      *(uintptr_t *)s.cursor = s.end;
      ...
      void *dest = NULL;
      status = allo_stack_alloc_unsafe(&dest, &s, size, aligns[align_i]);
...
static void test_allo_stack_free_unsafe_ok(void) {
  uint8_t buf[BUF_SIZE] = {0};
  allo_stack s = {0};
  allo_status status = allo_stack_init(&s, buf, BUF_SIZE);
  ALLO_TEST_ASSERT_STATUS_MSG(ALLO_OK, status, "init must succeed");
  ...
  for (size_t i = 0; i < ALLO_ARR_LEN(cursor_positions); ++i) {
    ALLO_TEST_ASSERT_STATUS_MSG(ALLO_OK, allo_stack_free_unsafe(&s),
                                "free must succeed");
    TEST_ASSERT_EQUAL_PTR_MESSAGE(
...
int main(void) {
  ...
  RUN_TEST(test_allo_stack_alloc_unsafe_empty_allocator);
  RUN_TEST(test_allo_stack_alloc_unsafe_non_empty_allocator);
  RUN_TEST(test_allo_stack_alloc_unsafe_oom);
  ...
  RUN_TEST(test_allo_stack_free_unsafe_ok);
